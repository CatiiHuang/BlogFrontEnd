  ...
  export default {
    ...
    data() {
      return {
        ...
        gist: '',
        content: '',
        categorys:['JavaScript','ES6','jQuery','css','ajax','Vue-router','Vuex',
        'http','https','websocket','apache','Vue','react','react-nactive','axios',
        'node','mongo','CentOS','webpack','学习笔记','Echarts','MySQL','Less','Linux',
        'three.js','d3.js','php','Electron','easyUI','ECharts','wx','HBuilder']
      }
    },
    ...
    methods: {
      $imgAdd(pos, $file) {
        console.log(pos);
        console.log($file);
        this.$axios.post(webUrl+"uploadImg",{
           data : $file.miniurl
          }
        ).then((res)=>{
          this.$refs.md.$img2Url(pos,res.data.data.toString());
        }).catch((res)=>{
          console.log(res);
        })
      },
