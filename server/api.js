...
//访问次数
router.get('/api/visitied',(req,res)=>{
  db.times.find((err,docs)=>{//查询是否有次数数据
    if (err) throw err;
    if (docs.length>0){//有就+1 并更新数据
      var times2 = (docs[0].times+1);
      db.times.updateOne({times:docs[0].times},{$set:{times:times2}},(err,docs2)=>{
        if(err) throw err;
        console.log("updata::",docs2);
...
router.post('/api/uploadImg',(req,res)=>{
  ...
  let D = Date.now();
  let saveImg = path.join(__dirname, '../static/upload/article/' + D + '.png');//api.js的上级的static下
  fs.writeFile(saveImg, dataBuffer, function(err) {
    if(err){
      res.send(err);
    }else{
      res.send({code:"200",data:"http://127.0.0.1:8080/"+"/static/upload/article/"+D+".png"});
    }
  });
});
